...
public class ReflectionUtils {
	...
	public static void assertPrivateAccessAllowed() {
		...
		if (manager != null) {
			try {
				...
				throw new ValidateException("Current security manager configuration does not allow access to private fields and methods.",
						ex);
			}
		}
	}
	 ...
	 *            whether to also return annotations declared on interface
	 *            declaration
	...
	/**
	 * @param setter
	 * @return the corresponding field for a setter method. Returns null if the
	 *         method is not a JavaBean style setter or the field could not be
	 *         located.
	 */
	public static Field getFieldForSetter(final Method setter) {
		...
		// calculate the corresponding field name based on the name of the
		// setter method (e.g. method setName() => field
		// name)
		...
		// if method parameter type is boolean then check if a field with name
		// isXXX exists (e.g. method setEnabled() =>
		// field isEnabled)
	...
	public static Method getGetter(final Class<?> clazz, final String propertyName) {
		final String appendix = propertyName.substring(0, 1).toUpperCase() + propertyName.substring(1);
		try {
	...
	public static <T> T invokeMethod(final Method method, final Object obj, final Object... args) {
		try {
			...
			throw new ValidateException("Executing method " + method.getName() + " failed.");
		}
	}
	 ...
	 * @param inspectInterfaces
	 *            whether to also check annotations declared on interface method
	 *            declaration
	 * @return true if an annotation for the specified annotation type is
	 *         present on this method, else false
	 */
